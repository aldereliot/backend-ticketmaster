...
public class HomeService {
    ...
    public HomeResponse getHomeData(){
        ...
        List<EventCardResponse> events = eventRepository
                .findHeroEvents(EventStatusEnum.PUBLISHED, LocalDateTime.now(), pageRequest);

        Map<String, List<EventCardResponse>> eventsByCategory = new HashMap<>();

        categories
                .forEach( category -> {
                    List<EventCardResponse> eventsCategory = eventRepository.findByCategoryName(EventStatusEnum.PUBLISHED, category.getName(), LocalDateTime.now(), PageRequest.of(0, 4));
